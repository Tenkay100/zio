  ...
  return {
    ...
    auth: {
      signUp: async ({ email, password, options }) => {
        try {
          const signUpPromise = window.firebase.auth().createUserWithEmailAndPassword(email, password);
          const timeoutPromise = new Promise((_, reject) => 
      ...
      signInWithPassword: async ({ email, password }) => {
        try {
          const signInPromise = window.firebase.auth().signInWithEmailAndPassword(email, password);
          const timeoutPromise = new Promise((_, reject) => 
            setTimeout(() => reject(new Error("Firebase timeout")), 1500)
          );
